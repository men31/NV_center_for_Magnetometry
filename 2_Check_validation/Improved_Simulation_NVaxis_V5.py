...
''' 
 ---------------- Updating in version 5 ------------------
 
 1) Changing the reference axis allowing NV-axis and NV-axis is described in term of matrix instead the vector.
 2) Setting inital Bval from 6 to 0
 3) Redefining Projection function
 4) Unlimiting the observation frame of theta from 0, 60 to 0, 180
 5) Starting to merge this file into Application file
...
class SimNV:
    def __init__(self):
        ...
        u = [u1_ori, u2_ori, u3_ori, u4_ori]

        # Rotated Reference Axis allowing NV axis
        u_axs = [self.MakeCoordinate_u(u_i) for u_i in u]
        self.u_dict = {f'NV_{i}': u_axs[i] for i in range(len(u_axs))}
    ...
    ''' Sub Program used to Manipulate with Class Objectives'''

    def MakeCoordinate_u(self, u):
        theta = find_theta(u, self.n_axs[:, 2].reshape(-1, 1))
        phi = find_phi(u, self.n_axs[:, 0].reshape(-1, 1), self.n_axs[:, 1].reshape(-1, 1))
        rot = self.rotate_mat(sum(theta), sum(phi)).astype(np.float64)
        u_axs = (rot @ self.n_axs)
        return u_axs
    ...
    @staticmethod
    def Projection_B2U(B_vec, u_axs):
        return u_axs @ B_vec
